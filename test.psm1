...
Function TRFormats {
    ...
    Write-Host "change the region settings to Turkiye?" -BackgroundColor Yellow -ForegroundColor Black -NoNewline
    Write-Host "(y/n): " -ForegroundColor Green -NoNewline
    $input = Read-Host
    if ($input -match "[Yy]") {
...
Function Defaultps1 {
    ...
    Set-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Internet Explorer\Main" -Name "DisableFirstRunCustomize" -Value 2 #Igrone IE First Settings
}
...
foreach ($package in $packages) {
    ...
    Invoke-Expression -Command "winget install $packageIdentifier -e --silent --accept-source-agreements --accept-package-agreements --force" *>$null
    Write-Host "[DONE]" -ForegroundColor Green -BackgroundColor Black
}
    Start-Sleep 5
    $progressPreference = 'silentlyContinue'
...
foreach ($vse in $vsextensions) {
    if ($installed.Contains($ext)) {
        Write-Host $vse "already installed." -ForegroundColor Gray
         ...
         code --install-extension $vse *>$null
    }
}
    Write-Host "[DONE]" -ForegroundColor Green -BackgroundColor Black
...
Stop-Service -Name $chromeservices -Force -ErrorAction SilentlyContinue
Set-Service -Name $chromeservices -Status stopped -StartupType disabled -ErrorAction SilentlyContinue
sc.exe delete $chromeservices *>$null
...
Function UninstallThirdPartyBloat {
    ...
    $progressPreference = 'SilentlyContinue'
    Get-AppxPackage -AllUsers $Uninstall3Party | Remove-AppxPackage | Out-Null -ErrorAction SilentlyContinue *>$null
    $progressPreference = 'SilentlyContinue'
    Get-AppxPackage $Uninstall3Party | Remove-AppxPackage | Out-Null -ErrorAction SilentlyContinue *>$null
...
Function Remove3D {
    ...
    PS C:\Windows\system32> Remove-Item -Path "HKLM:\SOFTWARE\Wow6432Node\Microsoft\Windows\CurrentVersion\Explorer\MyComputer\NameSpace\{0DB7E03F-FC29-4DC6-9020-FF41B59E513A}" -Recurse -ErrorAction SilentlyContinue
    Write-Host "[DONE]" -ForegroundColor Green -BackgroundColor Black 
}
...
Function UninstallEdge {
    ...
    Stop-Service -Name $edgeservices -Force -ErrorAction SilentlyContinue
    Set-Service -Name $edgeservices -Status stopped -StartupType disabled -ErrorAction SilentlyContinue
    sc.exe delete $edgeservices *>$null
...
$Shortcut.Save()
Unblock-File -Path "C:\after-format-main\files\icons\Sublime Text.lnk" *>$null
...
$Shortcut.Save()
Unblock-File -Path "C:\after-format-main\files\icons\TreeSize Free (Administrator).lnk" *>$null
...
$ShortcutFile = "C:\after-format-main\files\icons\PowerToys.lnk"
$Shortcut = $WScriptShell.CreateShortcut($ShortcutFile)
$Shortcut.TargetPath = $Powertoys
$Shortcut.WorkingDirectory = $PowertoysPath
$Shortcut.Save()
Unblock-File -Path "C:\after-format-main\files\icons\PowerToys.lnk" *>$null
