...
Function SystemSettings {
    ...
    if ($response -eq 'y' -or $response -eq 'Y') {
        ...
        Function NVCleanUpdateTask {
            ...
            try {
                $nvcleanstalldriverurl = "https://drive.usercontent.google.com/download?id=1GR6EN9lY2MP8rvJ0CYAF_YSRdmr9-3xK&export=download&confirm=t&uuid=3dafda5a-d638-4e45-8655-3e4dcc5a7212&at=APZUnTXgUibc057YzjK_mWRb_0Di%3A1713698912361"
                ...
                Silent
                Invoke-WebRequest -Uri $nvcleanstalldriverurl -Outfile "$nvcleanpath\NVCleanstall_NVIDIA_581.80_x64_dch_Desktop_Setup.exe" -ErrorAction Stop
                ...
                # Install NVIDIA
                Start-Process -FilePath "$nvcleanpath\NVCleanstall_NVIDIA_581.80_x64_dch_Desktop_Setup.exe" -ArgumentList "-y" -Wait
                Start-Sleep 2
                Remove-Item -Path $nvcleanpath\NVCleanstall_NVIDIA_581.80_x64_dch_Desktop_Setup.exe -Recurse -Force *>$null
            ...
            if ($response -eq 'y' -or $response -eq 'Y') {
                ...
                Function RightClickMenu {
                    try {
                        ...
                        $contextMenuPaths = @(
                            "HKCR:\AllFilesystemObjects\shellex\ContextMenuHandlers\SendTo", #remove send to
                            "HKCR:\UserLibraryFolder\shellex\ContextMenuHandlers\SendTo", #remove send to
                            "HKCR:\AllFilesystemObjects\shellex\ContextMenuHandlers\ModernSharing", #remove share
                            "HKCR:\*\shell\pintohomefile", #remove favorites
                            #remove give access
                            ...
                            #remove "copy as path"
                            "HKLM:\SOFTWARE\Classes\AllFilesystemObjects\shellex\ContextMenuHandlers\CopyAsPathMenu"
                            #remove git
                            "HKCR:\Directory\Background\shell\git_gui",
                            "HKCR:\Directory\Background\shell\git_shell",
                            #remove treesize
                            "HKCR:\Directory\Background\shell\TreeSize Free",
                            "HKCR:\Directory\Background\shell\VSCode"
                            #remove mpc player
                            "HKCR:\Directory\shell\mplayerc64.enqueue"
                            #remove sharex
                            "HKCR:\Directory\shell\ShareX"
                            #remove vlc
                            "HKCR:\Directory\shell\AddToPlaylistVLC"
                            #remove google drive
                            "HKCR:\GoogleDriveFS.gcsedoc"
                            "HKCR:\GoogleDriveFS.gcsesheet"
                            "HKCR:\GoogleDriveFS.gcseslides"
                            "HKCR:\GoogleDriveFS.gdoc"
                            "HKCR:\GoogleDriveFS.gdraw"
                            "HKCR:\GoogleDriveFS.gdrive"
                            "HKCR:\GoogleDriveFS.gform"
                            "HKCR:\GoogleDriveFS.gjam"
                            "HKCR:\GoogleDriveFS.glink"
                            "HKCR:\GoogleDriveFS.gmaillayout"
                            "HKCR:\GoogleDriveFS.gmap"
                            "HKCR:\GoogleDriveFS.gnote"
                            "HKCR:\GoogleDriveFS.gscript"
                            "HKCR:\GoogleDriveFS.gsheet"
                            "HKCR:\GoogleDriveFS.gsite"
                            "HKCR:\GoogleDriveFS.gslides"
                            "HKCR:\GoogleDriveFS.gtable"
                            "HKCR:\GoogleDriveFS.gvid"
                        )
                        ...
                        $regpath = "HKCR:\*\shell\hash"
                        $sha256menu = "HKCR:\*\shell\hash\shell\02menu"
                        $md5menu = "HKCR:\*\shell\hash\shell\03menu"

                        if (!(Test-Path -LiteralPath $regpath)) {
                            New-Item -Path $regpath -Force | Out-Null
                        }
                        Set-ItemProperty -LiteralPath $regpath -Name "MUIVerb" -Value "HASH" -ErrorAction SilentlyContinue
                        Set-ItemProperty -LiteralPath $regpath -Name "SubCommands" -Value "" -ErrorAction SilentlyContinue

                        if (!(Test-Path -LiteralPath "$regpath\shell")) {
                            New-Item -Path "$regpath\shell" -Force | Out-Null
                        }

                        # SHA256 menu
                        if (!(Test-Path -LiteralPath $sha256menu)) {
                            New-Item -Path $sha256menu -Force | Out-Null
                        }
                        Set-ItemProperty -LiteralPath $sha256menu -Name "MUIVerb" -Value "SHA256" -ErrorAction SilentlyContinue

                        if (!(Test-Path -LiteralPath "$sha256menu\command")) {
                            New-Item -Path "$sha256menu\command" -Force | Out-Null
                        }
                        $sha256cmd = 'powershell -noexit get-filehash -literalpath "%1" -algorithm SHA256 | format-list'
                        Set-ItemProperty -LiteralPath "$sha256menu\command" -Name "(Default)" -Value $sha256cmd -ErrorAction SilentlyContinue

                        # MD5 menu
                        if (!(Test-Path -LiteralPath $md5menu)) {
                            New-Item -Path $md5menu -Force | Out-Null
                        }
                        Set-ItemProperty -LiteralPath $md5menu -Name "MUIVerb" -Value "MD5" -ErrorAction SilentlyContinue

                        if (!(Test-Path -LiteralPath "$md5menu\command")) {
                            New-Item -Path "$md5menu\command" -Force | Out-Null
                        }
                        $md5cmd = 'powershell -noexit get-filehash -literalpath "%1" -algorithm MD5 | format-list'
                        Set-ItemProperty -LiteralPath "$md5menu\command" -Name "(Default)" -Value $md5cmd -ErrorAction SilentlyContinue

                        # Add Turn Off Display Menu
                        $turnOffDisplay = "HKCR:\DesktopBackground\Shell\TurnOffDisplay"
                        if (!(Test-Path $turnOffDisplay)) {
                            New-Item -Path $turnOffDisplay -Force | Out-Null
                        }
                        Set-ItemProperty -Path $turnOffDisplay -Name "Icon" -Value "imageres.dll,-109" -ErrorAction SilentlyContinue
                        Set-ItemProperty -Path $turnOffDisplay -Name "MUIVerb" -Value "Turn off display" -ErrorAction SilentlyContinue
                        Set-ItemProperty -Path $turnOffDisplay -Name "Position" -Value "Bottom" -ErrorAction SilentlyContinue
                        Set-ItemProperty -Path $turnOffDisplay -Name "SubCommands" -Value "" -ErrorAction SilentlyContinue

                        if (!(Test-Path "$turnOffDisplay\shell")) {
                            New-Item -Path "$turnOffDisplay\shell" -Force | Out-Null
                        }

                        $turnOffMenu1 = "$turnOffDisplay\shell\01menu"
                        if (!(Test-Path $turnOffMenu1)) {
                            New-Item -Path $turnOffMenu1 -Force | Out-Null
                        }
                        Set-ItemProperty -Path $turnOffMenu1 -Name "Icon" -Value "powercpl.dll,-513" -ErrorAction SilentlyContinue
                        Set-ItemProperty -Path $turnOffMenu1 -Name "MUIVerb" -Value "Turn off display" -ErrorAction SilentlyContinue

                        if (!(Test-Path "$turnOffMenu1\command")) {
                            New-Item -Path "$turnOffMenu1\command" -Force | Out-Null
                        }
                        $turnOffCmd1 = 'cmd /c "powershell.exe -Command \"(Add-Type ''[DllImport(\\"user32.dll\\")]public static extern int SendMessage(int hWnd,int hMsg,int wParam,int lParam);'' -Name a -Pas)::SendMessage(-1,0x0112,0xF170,2)\""'
                        Set-ItemProperty -Path "$turnOffMenu1\command" -Name "(Default)" -Value $turnOffCmd1 -ErrorAction SilentlyContinue

                        $turnOffMenu2 = "$turnOffDisplay\shell\02menu"
                        if (!(Test-Path $turnOffMenu2)) {
                            New-Item -Path $turnOffMenu2 -Force | Out-Null
                        }
                        Set-ItemProperty -Path $turnOffMenu2 -Name "MUIVerb" -Value "Lock computer and Turn off display" -ErrorAction SilentlyContinue
                        Set-ItemProperty -Path $turnOffMenu2 -Name "CommandFlags" -Value 0x20 -Type DWord -ErrorAction SilentlyContinue
                        Set-ItemProperty -Path $turnOffMenu2 -Name "Icon" -Value "imageres.dll,-59" -ErrorAction SilentlyContinue

                        if (!(Test-Path "$turnOffMenu2\command")) {
                            New-Item -Path "$turnOffMenu2\command" -Force | Out-Null
                        }
                        $turnOffCmd2 = 'cmd /c "powershell.exe -Command \"(Add-Type ''[DllImport(\\"user32.dll\\")]public static extern int SendMessage(int hWnd,int hMsg,int wParam,int lParam);'' -Name a -Pas)::SendMessage(-1,0x0112,0xF170,2)\" & rundll32.exe user32.dll, LockWorkStation"'
                        Set-ItemProperty -Path "$turnOffMenu2\command" -Name "(Default)" -Value $turnOffCmd2 -ErrorAction SilentlyContinue
                        ...
                        $rightclickregpath = @(
                            "HKCR:\Directory\shell\FindEmptyFolders",
                            "HKCR:\Directory\shell\FindEmptyFolders\command",
                            "HKCR:\Directory\Background\shell\FindEmptyFolders",
                            "HKCR:\Directory\Background\shell\FindEmptyFolders\command",
                            "HKCR:\Drive\shell\FindEmptyFolders",
                            "HKCR:\Drive\shell\FindEmptyFolders\command"
                        )
                        ...
                        $rightclickregpath | ForEach-Object {
                            ...
                            Set-ItemProperty -Path $_ -Name "(Default)" -Value $defaultValue -ErrorAction SilentlyContinue
                            Set-ItemProperty -Path $_ -Name "Icon" -Value $icon -ErrorAction SilentlyContinue
                        }

                        Set-ItemProperty -Path "HKCR:\Directory\shell\FindEmptyFolders\command" -Name "(Default)" -Value $command -ErrorAction SilentlyContinue
                        Set-ItemProperty -Path "HKCR:\Directory\Background\shell\FindEmptyFolders\command" -Name "(Default)" -Value $command -ErrorAction SilentlyContinue
                        Set-ItemProperty -Path "HKCR:\Drive\shell\FindEmptyFolders\command" -Name "(Default)" -Value $command -ErrorAction SilentlyContinue

                        # Add blocked keys
                        $blockedkeyPath = "HKCU:\Software\Microsoft\Windows\CurrentVersion\Shell Extensions\Blocked"
                        if (!(Test-Path -Path $blockedkeyPath)) {
                            New-Item -Path $blockedkeyPath -Force | Out-Null
                        }

                        # Adding Boot to UEFI Firmware Settings
                        if (!(Test-Path "HKCR:\DesktopBackground\Shell\Firmware")) {
                            New-Item -Path "HKCR:\DesktopBackground\Shell\Firmware" -Force | Out-Null
                        }
                        Set-ItemProperty -Path "HKCR:\DesktopBackground\Shell\Firmware" -Name "Icon" -Value "bootux.dll,-1016" -ErrorAction SilentlyContinue
                        Set-ItemProperty -Path "HKCR:\DesktopBackground\Shell\Firmware" -Name "MUIVerb" -Value "Boot to UEFI Firmware Settings" -ErrorAction SilentlyContinue
                        Set-ItemProperty -Path "HKCR:\DesktopBackground\Shell\Firmware" -Name "Position" -Value "Top" -ErrorAction SilentlyContinue

                        if (!(Test-Path "HKCR:\DesktopBackground\Shell\Firmware\command")) {
                            New-Item -Path "HKCR:\DesktopBackground\Shell\Firmware\command" -Force | Out-Null
                        }
                        $firmwareCmd = 'powershell -windowstyle hidden -command "Start-Process cmd -ArgumentList ''/s,/c,shutdown /r /fw'' -Verb runAs"'
                        Set-ItemProperty -Path "HKCR:\DesktopBackground\Shell\Firmware\command" -Name "(Default)" -Value $firmwareCmd -ErrorAction SilentlyContinue

                        # Remove "Edit in Notepad"
                        Set-ItemProperty -Path $blockedkeyPath -Name "{CA6CC9F1-867A-481E-951E-A28C5E4F01EA}" -Value "Edit in Notepad" -ErrorAction SilentlyContinue

                        # Remove "Cast to Device"
                        Set-ItemProperty -Path $blockedkeyPath -Name "{7AD84985-87B4-4a16-BE58-8B72A5B390F7}" -Value "Play to Menu" -ErrorAction SilentlyContinue

                        # Remove "AskCopilot"
                        Set-ItemProperty -Path $blockedkeyPath -Name "{CB3B0003-8088-4EDE-8769-8B354AB2FF8C}" -Value "AskCopilot" -ErrorAction SilentlyContinue
